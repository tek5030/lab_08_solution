...
MobaPoseEstimator::estimate(const std::vector<cv::Point2f>& image_points, const std::vector<cv::Point3f>& world_points)
{
  ...
  std::vector<PoseMeasurement> measurements;
  measurements.reserve(num_inliers);
...
Eigen::Vector2d MobaPoseEstimator::toNormalized(const Eigen::Vector2d& pixel) const
{
  return (pixel - principal_point_).array() / focal_lengths_.array();
}

Sophus::SE3d
MobaPoseEstimator::optimize(const std::vector<PoseMeasurement>& measurements,
                            const Sophus::SE3d& initial_pose) const
{
  const size_t max_iterations = 3;
  const int measure_dim = PoseMeasurement::measure_dim;
  const int state_dim = PoseMeasurement::state_dim;
  ...
  while (iteration < max_iterations)
  {
    ...
    {
      const LinearizedPoseMeasurement linearization = measurements[j].linearize(current_state);
